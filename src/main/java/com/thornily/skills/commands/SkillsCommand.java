...
public class SkillsCommand implements CommandExecutor {
    ...
    private void openSkillsGUI(Player player) {
        ...
        for (Skill skill : Skill.values()) {
            ...
            ResultSet rs = plugin.getDatabase().executeQuery(
                "SELECT level, experience FROM player_skills WHERE uuid = '" + uuid + "' AND skill_name = '" + skill.name() + "'"
            );
            ...
            try {
                ...
                plugin.getLogger().severe("Failed to load skill data for " + skill.name());
            }
        }
    ...
    private ItemStack createSkillItem(Skill skill, int level, double experience) {
        ...
        if (meta != null) {
            ...
            lore.add("§7Experience: §b" + String.format("%.1f", experience) + "§7/§b" + String.format("%.0f", getRequiredXP(level)));
            lore.add("");
    ...
    private Material getSkillMaterial(Skill skill) {
        switch (skill) {
            ...
            case MAGIC: return Material.ENCHANTED_BOOK;
            case ARCHERY: return Material.BOW;
            case HUNTING: return Material.CROSSBOW;
