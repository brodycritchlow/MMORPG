...
public class QueryCommand implements CommandExecutor {
    ...
    public QueryCommand(SkillsPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    private void executeUpdateQuery(CommandSender sender, String query) {
        ...
        if (rowsAffected > 0) {
            sender.sendMessage("§aQuery executed successfully. " + rowsAffected + " rows affected.");
        } else {
            sender.sendMessage("§cQuery executed but no rows affected, or query failed. Check console.");
        }
    }
}
