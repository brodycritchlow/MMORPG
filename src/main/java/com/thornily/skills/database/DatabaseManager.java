...
public class DatabaseManager {
  ...
  public DatabaseManager(SkillsPlugin plugin) { this.plugin = plugin; }
  ...
  public void disconnect() {
    try {
      ...
      plugin.getLogger().log(Level.SEVERE,
                             "Could not disconnect from database.", e);
    }
  }

  public void createTables() {
    try {
      String createPlayerSkillsTable =
      ...
      PreparedStatement statement =
          connection.prepareStatement(createPlayerSkillsTable);
      statement.execute();
      statement.close();

      plugin.getLogger().info("Database tables created successfully");
    } catch (SQLException e) {
      plugin.getLogger().log(Level.SEVERE, "Failed to create database tables",
                             e);
    }
  }
  ...
  public int executeUpdate(String query) {
    try {
      ...
      plugin.getLogger().log(Level.SEVERE,
                             "Failed to execute update on database", e);
    }
    return 0;
  }
  ...
  public boolean isConnected() {
    try {
      ...
      plugin.getLogger().log(Level.SEVERE,
                             "Failed to check database connection", e);
    }
    return false;
  }
}
